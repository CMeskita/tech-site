          ...
          {planos.map((plano) => (
            <div
              ...
              className={`rounded-2xl shadow-lg p-6 border ${
                plano.destaque ? "border-indigo-600 scale-105" : "border-gray-200"
              }`}
            >
              <h3 className="text-xl font-semibold mb-2">{plano.nome}</h3>
              <p className="text-2xl font-bold text-indigo-600 mb-4">{plano.preco}</p>
              ...
              <ul className="space-y-2 text-left mb-6">
                ...
                <Button onClick={handleSelectPlan} bgcolor="bg-indigo-600" destaque={plano.destaque}>
                    Escolher {plano.nome}
                </Button>
            </div>
          ))}
